...
public class BrowserUnit {
    ...
    public static String queryWrapper(Context context, String query) {
        ...
        BrowserPref sp = BrowserPref.getInstance();
        String customSearchEngine = sp.customSearchEngine();
        if (!sp.contains(BrowserPref.KEY_CUSTOM_SEARCH_ENGINE_SWITCH)) {
            sp.setCustomSearchEngineSwitch(!TextUtils.isEmpty(customSearchEngine));
        }
        if (sp.customSearchEngineSwitch()) {
            return customSearchEngine + query;
        } else {
            int i = sp.searchEngine();
            switch (i) {
                case 1:
    ...
    public static void download(final Context context, final String url, final String contentDisposition, final String mimeType) {
        ...
        String text = context.getString(R.string.libbrs_dialog_title_download) + " - " + URLUtil.guessFileName(url, contentDisposition, mimeType);
        ...
        builder.setPositiveButton(android.R.string.ok, (dialog, whichButton) -> {
            try {
                ...
                Toast.makeText(context, context.getString(R.string.libbrs_app_error) + e.toString().substring(e.toString().indexOf(":")), Toast.LENGTH_LONG).show();
                e.printStackTrace();
            }
        });
        builder.setNeutralButton(R.string.libbrs_menu_share_link, (dialog, whichButton) -> {
    ...
    public static void clearData(Context context) {
        ...
        if (browserPref.clearCache()) {
            clearCache(context);
        }
        if (browserPref.clearCookie()) {
            clearCookie();
        }
        if (browserPref.clearHistory()) {
            clearHistory(context);
        }
        if (browserPref.clearIndexedDb()) {
            clearIndexedDB(context);
